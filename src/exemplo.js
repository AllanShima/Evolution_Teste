...
document.getElementById("desconectar-instancia").addEventListener("click", desconectarInstancia);
...
function conectarInstancia(){
    ...
    fetch('http://localhost:8080/instance/connect/exemplo', options1)
      ...
      .then(data => {
        const codigoPareamento = data.pairingCode; // Extraindo o código do json
...
// Desconectar a instância do whats

function desconectarInstancia(){
  const options = {method: 'DELETE', headers: {apikey: '4c3901cf-458c-41e1-9439-2e6055c7a5be'}};

  fetch('http://localhost:8080/instance/logout/exemplo', options)
    .then(response => response.json())
    .then(data => console.log(data.response))
    .catch(err => console.error(err));
}

// Apagar a instância do whats
